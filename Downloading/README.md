...
2. Locate and order the data you want. For regular users, this is limited to 5 images per day. Once an image is ordered, you'll get a confirmation email with the production order number and scene ID attached. Once the ordered scenes are processed, you'll get another email with the SFTP download information. This includes the "Product path", where your processed data is located. From here, you can manually download the images by navigating in e.g. FileZilla, or use the bash script "sftp_download.sh". For the latter, the following steps are required:
